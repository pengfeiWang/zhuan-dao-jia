...
/***/ function(module, exports, __webpack_require__) {
	...
	var init = function init(vm) {
	  utils.ajax({
	    ...
	    success: function success(res) {
	      if (res.rescode == 100) {
	        for (var i = 0, len = timinggrabData.length; i < len; i++) {
	          timinggrabData[i].overFlg = parseInt(res.data['overFlg' + (i + 1)], 10);
	          timinggrabData[i].desc = res.data['doTime4' + (i + 1)] + arrTip[i];
	        }
	      } else {
	        utils.dialog('暂无红包数据');
	      }
